...
Recovery ships a cut-down userland. `libpython3.10.dylib` from
...
(ci/verify_installed_wheel.py, venv, pip) imports curses, so they are never
bound.  The technique is mimalloc-pprof's `ci/patch_recovery_python.py`,
verified there against Python 3.12.11 inside Recovery; Bosn applies it to the
CPython 3.10 build so the wheel is exercised on its abi3-py310 floor.
...
command's size is fixed; libSystem is both shorter and guaranteed present.
    ...
    python3 ci/patch_recovery_python.py <libpython3.10.dylib>
"""
...
DYLIB_CMDS = (0x0C, 0x18, 0x1F, 0x23)
...
def patch(path: str) -> None:
    ...
    if len(data) < 32:
        raise SystemExit(f"not a 64-bit little-endian Mach-O: {path} (too short)")
