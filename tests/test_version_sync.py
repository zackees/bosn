...
def crates_io_version() -> str:
    """The crates.io `bosn`, amalgamated from the internal crates at release."""
    cargo = tomllib.loads((ROOT / "crates" / "bosn" / "Cargo.toml").read_text(encoding="utf-8"))
    version = cargo["package"]["version"]
    assert isinstance(version, str), "crates/bosn/Cargo.toml [package].version is not a literal"
    return version
...
def test_version_declarations_match() -> None:
    versions = {
        ...
        "crates/bosn/Cargo.toml": crates_io_version(),
